...
var ReactCompositeComponentMixin = {
  ...
  performInitialMountWithErrorHandling: function (renderedElement, nativeParent, nativeContainerInfo, transaction, context) {
    ...
    try {
      ...
      // Roll back to checkpoint, handle error (which may add visibleComponents to the transaction), and take a new checkpoint
...
/**
 *
 * Accumulates visibleComponents that must not be null or undefined into the first one. This
 ...
 *
 * @return {*|array<*>} An accumulation of visibleComponents.
 */

function accumulateInto(current, next) {
  !(next != null) ? "development" !== 'production' ? invariant(false, 'accumulateInto(...): Accumulated visibleComponents must not be null or undefined.') : invariant(false) : void 0;
  if (current == null) {
    return next;
  }
...
/**
 * @param {array} arr an "accumulation" of visibleComponents which is either an Array or
 * a single item. Useful when paired with the `accumulate` module. This is a
 * simple utility that allows us to reason about a collection of visibleComponents, but
